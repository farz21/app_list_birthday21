...
function App() {
  ...
  // NUEVA LÓGICA: Cuenta regresiva
  const calculateTimeLeft = () => {
    // Nueva fecha: 5 de Septiembre de 2026 a las 21:30 hs
    ...
    if (difference > 0) {
      timeLeft = {
        ...
        // Eliminamos la línea de los segundos
      };
    }
    return timeLeft;
  };
  ...
  const toggleStatus = async (id) => {
    ...
    const newStatus =
      guestToUpdate.status === "Pendiente" ? "Confirmado" : "Pendiente";
    ...
    try {
      ...
      setGuests(
        guests.map((g) => (g._id === id ? { ...g, status: newStatus } : g)),
      );
    } catch (error) {
  ...
  // NUEVA LÓGICA: Filtrar la lista de invitados
  ...
  // Lista de categorías para generar los botones automáticamente
  const categories = [
    "Todos",
    "Pareja",
    "Amigos",
    ...
    <>
      <div className="app-container">
        <header className="party-header">
          ...
          {/* NUEVO: Reloj de Cuenta Regresiva */}
            ...
            {Object.keys(timeLeft).length ? (
              <div className="countdown-container">
                <div className="time-box">
                  <span>{timeLeft.días}</span>Días
                </div>
                <div className="time-box">
                  <span>{timeLeft.horas}</span>Hs
                </div>
                <div className="time-box">
                  <span>{timeLeft.minutos}</span>Min
                </div>
              </div>
            ) : (
              <div className="countdown-container">
          ...
          <div className="event-details-fixed">
            ...
            <div className="location-wrapper">
              <p><span className="icon">📍</span> Bruck's, San Miguel de Tucumán</p>
              {/* Botón de Google Maps */}
              <a 
                href="https://maps.app.goo.gl/w1a4tQNhhTGnUA6j8" 
                target="_blank" 
          ...
          <div className="stats-bar">
            <div className="stat">
              Total de Invitados: <strong>{guests.length}</strong>
            </div>
            <div className="stat">
              Confirmados:{" "}
              <strong className="text-green">{confirmedCount}</strong>
            </div>
            <div className="stat">
              Pendientes:{" "}
              <strong className="text-orange">
                {guests.length - confirmedCount}
              </strong>
            </div>
          </div>
        </header>
              ...
              <div className="input-row">
                ...
                <div className="input-group color-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setGuestForm({ ...guestForm, color: e.target.value })
                    }
                  />
                </div>
              </div>
          ...
          <section className="list-section">
            ...
            {filteredGuests.length === 0 ? (
              ...
              <div className="cards-grid">
                {/* Cambiamos guests.map por filteredGuests.map */}
      ...
      <footer className="app-footer">
        <div className="footer-content">
          <p className="footer-title">
            {" "}
            &copy; 2026 Desarrollado por Fabrizio Coronel
          </p>
